...
export async function POST(req: Request) {
  try {
    ...
    const {tokenId, mouthTokenId, headTokenId} = body
    ...
    if (
      ...
      typeof mouthTokenId !== 'number' ||
      typeof headTokenId !== 'number'
    ) {
    ...
    // Step 4: Write transaction to equip item
    const tx = await walletClient.writeContract({
      abi: operatorAbi as Abi,
      address: OPERATOR_ADDRESS,
      functionName: 'equipItem',
      args: [tokenId, mouthTokenId, headTokenId, signature],
    })
    ...
    console.error('Error in equipItem API:', error)
    return NextResponse.json({error: 'Failed to execute equipItem.', details: error}, {status: 500})
  }
}
